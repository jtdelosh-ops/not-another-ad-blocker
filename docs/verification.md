...
The opt-in [DNS development executable](dns-core.md) is implemented locally with no system-setting changes. Rust formatting and all **88 Rust tests** pass (76 library, 3 DNS CLI, 6 DNS socket integration, 3 native-host process), as do the **8 existing installer/native-client tests**. The 32 new DNS tests cover normalization, rule/exception precedence, unsupported context and preprocessing safety, bounded coverage reports, TTL/negative-cache expiry, packet/cache bounds, real UDP/TCP fallback, upstream failures, invalid messages, concurrency, command-line lifecycle and shutdown. The browser extension source did not change during this milestone; its earlier 71-test result is recorded below.
